...
public class CoverTree<E> {
	 ...
	 */
	public CoverTree (String measure) {
	 ...
	 */
	public CoverTree (double base, int maxMinLevel, String measure) {
		this.base = base;
	...
	void insertAtRoot(E element, double[] point) {
		...
		double dist = oldRoot.distance(point, measure);
	...
	public boolean insert(E element, double[] point) {
		...
		rootNode.distance = rootNode.distance(point, measure);
			...
			for (Node<E> n1 : coverset) {
				for (Node<E> n2 : n1.getChildren()) {
					if (n1.point!=n2.point) {
						...
						n2.distance = n2.distance(point, measure) ;
		...
		if (coverset.size()<numCenters) {
			...
			for (Node<E> n1 : candidates) {
				...
				for (Node<E> n2 : n1.getParent().getParent().getChildren()) {
					double dist = n1.distance(n2.point, measure);
					if (dist < minDist)
						minDist = dist;
				}
				n1.distance = minDist;
			...
			do {
				...
				for (Node<E> n : newNode.getParent().getParent().getChildren()) {
					if (n!=newNode) {
						double dist = newNode.distance(n.point, measure);
	 ...
	 * @return
	 */
	public Node<E> getNearest(double[] point) {
		List<Node<E>> candidates = new LinkedList<Node<E>>();
	...
	static double distance(double[] d1, double[] d2, String measure) {
	...
	class Node<E> implements Comparable<Node<E>> {
		...
		double distance(double[] point, String measure) {
			if(measure.equals("Euclidean"))
			{
				double sumSq = 0.;
